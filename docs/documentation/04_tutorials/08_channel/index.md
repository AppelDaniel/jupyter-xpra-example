...
```{code-cell}
./build/bin/posti_channel_fft parameter_channelfft.ini Re180_turbulentChannel_State_000001*.h5
```
The [parameter file](../../../../tutorials/channel/parameter_channelfft.ini) `parameter_channelfft.ini` is provided in the tutorial folder. The selection of the specific *State* files to be used is left to the user. The above command uses all state files with a timestamp between $t=10.0$ and $t=15.0$. As an output you receive three files. One contains the mean velocity profiles as well as the Reynolds stress profiles while the other two files contain turbulent energy spectra. To visualize those files, you can run the python script `plotChannelFFT.py`, provided in the ``tools/testcases`` folder with the following command in your simulation directory.
```{code-cell}
python ../../tools/testcases/plotChannelFFT.py -p Re180_turbulentChannel -t 10
```
Here, `-p` specifies the project name specified in the `parameter_flexi.ini` file and `-t` is the timestamp of your output files from the `posti_channel_fft` tool.
...
```{code-cell}
cd $FLEXI_ROOT
cmake -DFLEXI_PERFORMANCE=ON -DFLEXI_PERFORMANCE_OPTLIFT=ON -DFLEXI_PERFORMANCE_PGO=ON -DCMAKE_BUILD_TYPE=Profile --preset channel
cmake --build --preset channel
```
...
```{code-cell}
cd $FLEXI_ROOT
cmake -DFLEXI_PERFORMANCE=ON -DFLEXI_PERFORMANCE_OPTLIFT=ON -DFLEXI_PERFORMANCE_PGO=ON -DCMAKE_BUILD_TYPE=Release --preset channel
cmake --build --preset channel
```
