...
```{code-cell}
cd $FLEXI_ROOT
cmake --preset naca0012
cmake --build --preset naca0012
cd tutorials/naca0012
```
...
```
The chosen velocity vector $(u,v)^T$ yields an angle of attack of $\alpha=8^\circ$ and a velocity magnitude of $1$. The `RefState` are numbered in the order they are supplied in the **FLEXI** [parameter file](../../../../tutorials/naca0012/parameter_flexi.ini). Material properties are selected as described above. Based on the ideal gas law, we get
```{math}
...
We proceed by running the code in parallel. Here, `-np 4` instructs **FLEXI** to use four processes in parallel.
```{code-cell}
mpirun -np 4 ./build/bin/flexi parameter_flexi.ini
```
...
```{code-cell}
mpirun -np 4 ./build/bin/posti_visu parameter_postiVisu.ini parameter_flexi.ini NACA0012_Re5000_AoA8_State_0000000.0*
```
...
```{code-cell}
./build/bin/posti_preparerecordpoints parameter_recordpoints.ini
```

A sample [parameter file](../../../../tutorials/naca0012/parameter_recordpoints.ini) is included in the NACA0012 tutorial folder. Define the mesh and project name in this file. The options `NSuper` and `maxTolerance` adjust the record point search algorithm; setting `NSuper` to at least twice the mesh's polynomial degree is a good starting point. The remainder of the [parameter file](../../../../tutorials/naca0012/parameter_recordpoints.ini) is the actual definition of the record points.
...
### Record Points Usage
To utilize record points during a simulation, simply set a few options in the **FLEXI** [parameter file](../../../../tutorials/naca0012/parameter_flexi.ini). The essential options are given in the following.
```ini
RP_inUse            = T
...
For example, we want to calculate the mean flow $\bar{U}$ and the temporal fluctuations $U$) at our two record point planes. A sample [parameter file](../../../../tutorials/naca0012/parameter_visualizeRecordpoints.ini) `parameter_visualizeRecordpoints.ini` for the tool is provided in the tutorial folder. The options set in this file are
```ini
...
U' = U - \bar{U}.
```
We did not specify any specific variables for visualization, meaning all conservative variables stored in the *RPSet.h5* files will be utilized. If you wish to visualize a specific or derived variable (e.g., pressure), you can simply set it in the [parameter file](../../../../tutorials/naca0012/parameter_visualizeRecordpoints.ini) as follows
```ini
VarName              = Pressure
```
You can execute the tool using the command
```{code-cell}
./build/bin/posti_visualizerecordpoints parameter_visualizeRecordpoints.ini NACA0012_Re5000_AoA8_RP_*
```
