...
cd $FLEXI_ROOT
cmake --preset sod
cmake --build --preset sod
cd tutorials/sod
```

## Mesh Generation
In the tutorial directory, we provide the necessary mesh files, along with a [parameter file](../../../../tutorials/sod/parameter_hopr.ini) for **HOPR** to generate these meshes. You can recreate any mesh by running the following command. A full tutorial on how to run **HOPR** is available at the [HOPR documentation](https://hopr.readthedocs.io/en/latest/).
...
The [parameter file](../../../../tutorials/sod/parameter_flexi.ini) to run the simulation is supplied as `parameter_flexi.ini`. 
...
```{code-cell}
./build/bin/posti_visu parameter_postiVisu.ini parameter_flexi.ini sod_State_0*
```
